...
if [ "$2" == "" ]; then
	...
	rpmFileLocation=$workspace/target/rpm/ERICcaptool-CXC1734185/RPMS/x86_64
...
function runMaven {
    mvn -f $workspace/pom.xml -Droot.path=$workspace clean package -Drstate=$rstate 
}

cleanup
getProductNumber
setRstate
pkgName="captool_${rstate}.tar.gz"
git clean -df
git checkout $branch
git pull
